...
names=(
  "tablerock-${version}-aarch64-apple-darwin.tar.gz"
  "tablerock-${version}-x86_64-apple-darwin.tar.gz"
  "tablerock-${version}-aarch64-unknown-linux-gnu.tar.gz"
  "tablerock-${version}-x86_64-unknown-linux-gnu.tar.gz"
  "tablerock-app-${version}-aarch64-apple-darwin.zip"
)
: > "$tmp/assets.jsonl"
...
jq -Sn --arg source_repository tailrocks/tablerock --arg source_ref refs/tags/v$version \
...
(
  ...
  grep -F 'version "1.2.3"' Formula/tablerock.rb
  grep -F 'version "1.2.3"' Casks/tablerock-app.rb
  test "$(grep -h -c 'sha256 "[0-9a-f]\{64\}"' Formula/tablerock.rb Casks/tablerock-app.rb | awk '{n+=$1} END{print n}')" -eq 5
)
...
if (cd "$tmp/repo" && VELNOR_VERIFIED_PACKAGE_DIR="$verified" ./scripts/package-update.sh); then
  echo "mismatched source commit was accepted" >&2
  exit 1
fi
